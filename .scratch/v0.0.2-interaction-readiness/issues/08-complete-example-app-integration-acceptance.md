...
**Status:** partial (2026-09-02) — 主导航 fixture wiring 已按入口审计补齐；真机/Simulator 视觉与真网矩阵因 ExampleApp 构建环境 blocker 未执行。证据：`docs/qa/InkMarkdown-interaction-acceptance-2026-09-02.md`

- [x] ExampleApp 当前主导航可访问列表、复杂表格、表格复制、正文/表格/streaming 链接、inline/block 图片与失败 fallback 场景。（源码 + fixture wiring）
...
- [x] 第三方图片服务故障记录为环境失败，不通过放宽图片资源安全边界掩盖。（规则已写入证据；本轮未跑到网络步骤）
- [x] 保留所有已有 VoiceOver 行为；不新增完整朗读、表格无障碍树、VoiceOver UI test 或系统焦点编排。
- [x] UI、视觉和真实交互只做手工验收；禁止新增 UI automation、视觉 snapshot 或纯视觉单元测试。
- [x] 不把当前 runtime 结果宣称为 iOS/iPadOS 14–15 最低版本或真机性能证据。
