...
- [ ] 顶层 prepared filter 一次、raw 每 replace 每 cell 一次，resize 不增。
- [ ] makeView 与 updateExistingView 仍返回/接受原宿主类型。
- [ ] sizeThatFits 正值、width 协商、copy 手势不重复。
- [ ] 富文本链接和 ragged 行 corpus 通过。
