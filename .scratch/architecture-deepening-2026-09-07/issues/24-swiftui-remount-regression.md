...
- [x] 真实 SwiftUI remount 后新 delta 可见，原有 Thought live 状态按同周期规则保留。
- [x] 不同 Session 不继承旧内容或 pending environment。
- [x] reset/cancel 前排队的 promotion 不复活旧终态。
- [x] 测试不依赖截图像素、网络、固定长延时。
