...
- [ ] 视图删除 preparedHeaders/preparedReferenceRows/preparedRows 的并行所有权。
- [ ] 相同文本作为 reference 和 body 各处理一次，总计数符合输入事件。
- [ ] reference 不显示、不计数、不复制。
- [ ] 公开 setHeaders/appendRow 的顺序与分隔行行为保持。
