...
- [ ] 缓存命中不闪 loading placeholder、尺寸紧凑。
- [ ] prepareForReuse 后 A 晚到成功或失败都不能更新清空/新配置的 view。
- [ ] pending、失败 fallback、tap 回调和类型兼容保持。
- [ ] Block 不再 switch loading/queued 或直接保存 Store subscription。
