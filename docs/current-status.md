...
| 自定义扩展能力 | 源码预清洗、自定义行内语法扩展、自定义块级组件路由、链接点击回调 | `InkConfiguration` |
...
| 删除线 | 已渲 `.strikethroughStyle`；但 `inlineSyntaxes` 命中时（如 `@提及`）不继承删除线，详见 [spec](spec/extended-syntax.md) |
...
| 图片 / 删除线 | **ADR-004**：v1 仅占位 / 不承诺删除线样式 | 图片仍占位；**删除线已实现**（`.strikethroughStyle`，`inlineSyntaxes` 命中除外） | 删除线已补 spec；图片实现时再补契约 |
