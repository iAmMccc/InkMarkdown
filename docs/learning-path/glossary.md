...
实现 `InkBlockHandler` 的路由对象。它识别某个顶层 Markup 节点，并把它变成独立 `InkRenderableBlock`。见[第 5 章](05-block-and-stream-rendering.md)。

### CADisplayLink

跟随屏幕刷新节奏回调的定时机制。`InkStreamRenderer` 用它在主线程逐帧推进可见字符，不负责 Markdown 解析。
...
实际用于排版和绘制的字形。字符与 glyph 不保证永远一一对应，因此 TextKit 会区分字符范围与 glyph 范围。
...
只重做新内容或受影响范围，而不是每次把所有工作从头执行。InkMarkdown 流式实现缓存稳定前缀，并重算活跃后缀。
...
TextKit 1 的布局与绘制核心。它把字符映射到 glyph、计算行片段并绘制。项目子类 `InkMarkdownLayoutManager` 添加代码背景和引用线。

### NSAttributedString

“字符串 + 作用范围 + 属性”的富文本值。它保存内容和语义样式，不单独负责屏幕布局。

### NSRange

Foundation 使用的“起点 + 长度”范围。处理 Swift `String` 时应使用正确转换，不要凭 `String.count` 手算 Emoji 范围。

### NSTextContainer

TextKit 1 中描述文字可用布局区域的对象，例如容器宽度和行片段内边距。

### NSTextStorage

TextKit 1 中的可变富文本存储。内容或 attributes 改变时，它会通知关联的布局管理器。

## O–S

### Paragraph style

段落级排版属性集合，例如固定行高、首行缩进、段后距和对齐。Foundation 类型为 `NSParagraphStyle` / `NSMutableParagraphStyle`。
...
流式输入中，边界之前的内容被当前算法认为不会再受后续字符影响。项目采用保守判断，不等于通用增量 CommonMark 标准。

### Stable prefix / 稳定前缀

流式累计文本中可以缓存、不必每次重新渲染的前半部分。
...
流式累计文本中仍可能被未来字符改变的尾部。例如未闭合的加粗、链接或代码围栏。

### TextKit 1

Apple 的经典文本布局系统，核心链路是 `NSTextStorage → NSLayoutManager → NSTextContainer`。InkMarkdown 当前正式使用这条路径。
