...
本清单用于维护者判断某个提交是否可以成为 v0.0.2 Release Candidate，以及后续是否可以发布。勾选清单不授权 push、创建 Pull Request、合并、创建 tag 或发布 GitHub Release；按维护者现有授权执行，不因本清单追加重复确认。
...
- [ ] Release Candidate 验证开始后，不再修改候选提交；任何提交都会产生新 SHA；重新记录候选身份，并按实际影响决定重跑范围。
...
- [ ] 五个 product 均能由外部消费者独立导入和构建：`InkMarkdown`、`InkMarkdownSwiftUI`、`InkMarkdownLaTeX`、`InkMarkdownMermaid`、`InkMarkdownKingfisher`。
- [ ] Core 消费者不被迫链接 iosMath 或 Mermaid 资源；optional addon 的依赖和资源没有泄漏到 Core。

## 3. 公开 API 与兼容性

- [ ] 已将候选公开 interface 与已发布 tag `0.0.1` 比较；列明兼容性变化。ADR-012 授权的图片后端破坏式迁移须有准确说明和消费者验证；其余 API 按既定兼容性约束审计，不承诺 binary compatibility。
...
- [ ] 依赖升级已在 iOS Simulator 上重跑受影响测试与五个 product 消费者构建。
- [ ] 仓库与发布说明未包含三方缓存源码、构建产物或不应再分发的测试资源。

## 5. 自动化验证

优先使用 XcodeBuildMCP 发现可用 scheme 与 iOS Simulator destination。MCP 不可用时，说明工具限制并使用已安装 CLI 或原生 `xcodebuild`；不要把 macOS host 上 `swift test` 的 `no such module 'UIKit'` 当作库失败。

- [ ] `InkMarkdown-Package` 全量测试通过；只允许已记录且与候选范围无关的 skip。
- [ ] Core、SwiftUI adapter、addon contract/LaTeX 与确定性 Mermaid 关键路径分别通过。
- [ ] 已审计必要语义矩阵：静态、流式、configuration、自定义扩展、opt-in capabilities（图片后端与呈现订阅、取消、缓存与安全策略，以及 LaTeX/Mermaid addons），以及 session 结束、取消、重置、重新绑定和 promotion 生命周期关键契约均有数据、状态或语义证据；纯视觉、交互与非核心路径不扩张单测。
- [ ] ExampleApp app-hosted Mermaid PNG 关键链路通过；不在无 App 生命周期的 SwiftPM runner 中执行真实 WKWebView PNG 回归。
- [ ] 外部 consumer fixture 的五个单 product scheme 分别构建通过。
...
- [ ] 没有用 rerun、超时增加或测试删除掩盖真实失败；修复生成新候选 SHA 后，重新完成受影响检查和仓库要求的门禁。
- [ ] 同一 SHA 的 Standards 与 Spec 复审无未解决 P1/P2。

## 10. 发布操作

以下发布操作需要维护者授权；若会话已覆盖具体操作，不重复索取批准。
...
本节不复制易漂移的测试计数或 blocker 清单。当前 v0.0.2 尚未满足全部发布门槛；未完成项以 [`current-status.md`](current-status.md)与 [`roadmap.md`](roadmap.md) 为准。任何未勾选门禁都不能被基础契约通过替代。
