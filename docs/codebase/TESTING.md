...
| Item | Value | Evidence |
|------|-------|----------|
| Framework | **Swift Testing**（非 XCTest 为主） | `import Testing`、`@Test`、`@Suite` |
...
| Coverage gate | 未发现强制 coverage 阈值文件 | scan / 仓库根 |
...
| `StreamingPerformanceTests.swift` | 增量 vs 全量输出一致；增量耗时 ≤ 全量 30% 闸门 |
| `Snapshots/RenderSnapshot.swift` | 快照模型 + `RenderContractAssertions` 助手 |
| `Snapshots/SnapshotScaffoldTests.swift` | 快照基建冒烟（全集语义矩阵地基） |
...
| `@_spi(Performance) @testable import` | 性能基准 SPI | 行高与流式性能相关测试 |
...
**反模式：** 在 macOS host 执行 `swift test` 并因 `no such module 'UIKit'` 判定库逻辑失败。
...
- `InkStreamingPerformanceBenchmark.measure()` 在 `@_spi(Performance)` 下暴露。
- 阈值注释写明：默认数据集 incremental/full ≈ 0.15–0.19；上限 **0.30** 吸收模拟器方差。
- 性质是**回归闸门**，不是微基准排行。
