...
public struct InkConfiguration: Sendable {
  ...
  ///
  /// 比较策略：样式与环境按值比较；行内语法与块处理器逐项按**动态类型**比较
  /// （协议未暴露名称或状态，动态类型是唯一可靠代理；顺序影响渲染输出，故保留顺序）；
  /// 闭包（``sourceFilter`` / ``linkTapHandler``）在 Swift 中无法比较相等性，
  /// 仅比较存在性，避免"含闭包即不等价"引发 SwiftUI Coordinator 无限重渲染
  /// （见 docs/contributor-guide/09-swiftui-uiviewrepresentable-gotchas.md §6.2）。
  public func isSemanticallyEqualTo(_ other: InkConfiguration) -> Bool {
    ...
    zip(inlineSyntaxes, other.inlineSyntaxes).allSatisfy { lhs, rhs in
      String(describing: type(of: lhs)) == String(describing: type(of: rhs))
    } &&
    blockHandlers.count == other.blockHandlers.count &&
    zip(blockHandlers, other.blockHandlers).allSatisfy { lhs, rhs in
      String(describing: type(of: lhs)) == String(describing: type(of: rhs))
    } &&
